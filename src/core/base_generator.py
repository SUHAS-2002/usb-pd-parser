...
class BaseGenerator(ABC):
    ...
    def __str__(self) -> str:
        ...
        return f"{self.__class__.__name__}(count={self.__generated_count}, output={self.__output_path})"
    ...
    def __eq__(self, other: object) -> bool:
        """Equality based on class and generation count."""
        if not isinstance(other, BaseGenerator):
            return NotImplemented
        return (
            self.__class__ == other.__class__
            and self.__generated_count == other.__generated_count
        )
