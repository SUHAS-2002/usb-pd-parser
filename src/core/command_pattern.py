...
class ExtractCommand(Command):
    ...
    def execute(self) -> List[Dict]:
        ...
        if hasattr(self.__extractor, 'extract'):
            self.__result = self.__extractor.extract({"pdf_path": self.__pdf_path})
        else:
            self.__result = []
    ...
    def __str__(self) -> str:
        """Human-readable representation."""
        return f"ExtractCommand(pdf={self.__pdf_path}, result={len(self.__result) if self.__result else 0})"
...
class CommandInvoker(BaseInvoker):
    ...
    def __init__(self) -> None:
        ...
        super().__init__()  # Initialize BaseInvoker
    ...
    def __str__(self) -> str:
        """Human-readable representation."""
        return f"CommandInvoker(executed={self.__executed_count}, history={len(self.__command_history)})"
