...
class SectionRecordBuilder(Builder):
    ...
    def build(self) -> Dict[str, Any]:
        ...
        if not all(
            [
                ...
                self.__section_id,
                self.__title,
                self.__page is not None,
            ]
        ):
            raise ValueError(
                "Missing required fields for section record"
            )
        ...
        return {
            ...
            "level": (
                self.__level
                or (self.__section_id.count(".") + 1)
            ),
            "parent_id": (
                self.__parent_id
                or (
                    self.__section_id.rsplit(".", 1)[0]
                    if "." in self.__section_id
                    else None
                )
            ),
    ...
    def __str__(self) -> str:
        ...
        return (
            "SectionRecordBuilder("
            f"section_id={self.__section_id}, "
            f"title={self.__title})"
        )

    def __repr__(self) -> str:
        """Developer-friendly representation."""
        return "SectionRecordBuilder()"
