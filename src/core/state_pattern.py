...
class State(ABC):
    ...
    def handle(self, context: "StateContext") -> None:
        """Handle state-specific behavior (must be implemented by subclasses)."""
...
class InitialState(State):
    ...
    def handle(self, context: "StateContext") -> None:
        ...
        context.__state_data["transitions"] = context.__state_data.get("transitions", 0) + 1
...
class BaseStateContext(ABC):
    ...
    def set_state(self, state: State) -> None:
        """Change state (must be implemented by subclasses)."""
        raise NotImplementedError
...
class StateContext(BaseStateContext):
    ...
    def __init__(self) -> None:
        ...
        super().__init__()  # Initialize BaseStateContext
    ...
    def __str__(self) -> str:
        """Human-readable representation."""
        return f"StateContext(state={self.__current_state.__class__.__name__}, transitions={self.__state_data.get('transitions', 0)})"
    
    def __repr__(self) -> str:
        """Developer-friendly representation."""
        return f"StateContext()"
    ...
    def __contains__(self, state: State) -> bool:
        ...
        return any(type(s) == type(state) for s in self.__state_history)
